...
def create_app() -> FastAPI:
    ...
    from app.version import __brand__, __parent_brand__, __codename__
    ...
    app = FastAPI(
        title=f"{__brand__} API - {__parent_brand__}",
        description=f"{__brand__} by {__parent_brand__} - {__codename__}. Veiled Connectivity, Flux Intelligence. Next-gen proxy orchestration with masked identities, ghost routing, and adaptive flux balancing.",
        version=__version__,
        lifespan=lifespan,
        openapi_url="/openapi.json" if runtime_settings.docs else None,
        docs_url=None,
        contact={"name": "RunoFlux", "url": "https://github.com/amirparsa1/MaskPanel"},
    )
    ...
    on_startup(lambda: logger.info(f"MaskPanel by RunoFlux v{__version__} [{__codename__}] ({runtime_settings.role.value}) - Veiled Flux Active"))
