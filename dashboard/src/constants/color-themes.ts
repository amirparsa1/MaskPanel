export type BaseColor = 'default' | 'zinc' | 'slate' | 'stone' | 'gray' | 'mauve' | 'olive' | 'mist' | 'obsidian' | 'flux'
export type ColorTheme = 'default' | 'inverse' | 'red' | 'rose' | 'pink' | 'orange' | 'amber' | 'yellow' | 'green' | 'teal' | 'cyan' | 'blue' | 'indigo' | 'violet' | 'flux' | 'mask' | 'rune'
export type ThemeStyle = 'vega' | 'nova' | 'maia' | 'lyra' | 'mask' | 'flux'
...
export const baseColors: Record<BaseColor, ColorThemeConfig> = {
  default: basePalette('RunoFlux Default', 262, 15, 8, '262 83% 58%', '0 0% 98%', '262 83% 64%', '240 15% 6%'),
  ...
  obsidian: {
    ...
    dark: {
      ...
      '--sidebar-accent-foreground': '250 20% 96%',
  ...
  flux: {
    ...
    light: {
      ...
      '--foreground': '240 15% 10%',
      '--secondary': '190 90% 88%',
      '--secondary-foreground': '190 90% 15%',
      '--muted': '250 20% 92%',
      ...
      '--chart-4': '190 70% 60%',
      '--chart-5': '280 60% 70%',
      '--sidebar-background': '250 25% 95%',
      '--sidebar-foreground': '240 15% 18%',
      '--sidebar-primary': '262 83% 58%',
      '--sidebar-primary-foreground': '0 0% 98%',
      '--sidebar-accent': '250 20% 88%',
      '--sidebar-accent-foreground': '240 15% 10%',
      '--sidebar-border': '250 15% 84%',
      '--sidebar-ring': '262 83% 58%',
    },
    dark: {
      ...darkShared,
...
export const accentThemes: Record<ColorTheme, ColorThemeConfig> = {
  ...
  default: { name: 'RunoFlux Default', light: {}, dark: {} },
  ...
  flux: accentOverlay('Flux', '262 83% 58%', '0 0% 98%', '262 83% 64%', '240 15% 6%', ['262 83% 58%', '190 94% 43%', '280 85% 65%', '200 90% 60%', '262 70% 45%'], ['262 83% 64%', '190 94% 48%', '280 85% 70%', '200 90% 65%', '262 70% 55%']),
  mask: accentOverlay('Mask', '240 15% 15%', '0 0% 98%', '250 20% 96%', '240 15% 6%', ['240 15% 15%', '240 10% 30%', '240 8% 45%', '240 6% 60%', '240 5% 75%'], ['250 20% 96%', '250 15% 85%', '250 12% 70%', '250 10% 55%', '240 8% 40%']),
  rune: accentOverlay('Rune', '38 92% 50%', '0 0% 98%', '38 92% 55%', '240 15% 6%', ['38 92% 50%', '25 95% 53%', '15 90% 50%', '48 96% 53%', '38 80% 40%'], ['38 92% 55%', '25 95% 60%', '15 90% 58%', '48 96% 60%', '38 80% 55%']),
}

export const baseColorOrder: BaseColor[] = ['default', 'obsidian', 'flux', 'zinc', 'slate', 'stone', 'gray', 'mauve', 'olive', 'mist']
export const colorThemeOrder: ColorTheme[] = ['default', 'flux', 'mask', 'rune', 'violet', 'cyan', 'blue', 'indigo', 'teal', 'green', 'amber', 'orange', 'red', 'rose', 'pink', 'inverse']
...
export const themeStylePresets: Record<ThemeStyle, { density: 'compact' | 'comfortable' | 'spacious'; surface: 'flat' | 'subtle' | 'elevated'; radius: string }> = {
  ...
  mask: { density: 'comfortable', surface: 'elevated', radius: '0.75rem' },
  flux: { density: 'comfortable', surface: 'elevated', radius: '0.75rem' },
}
