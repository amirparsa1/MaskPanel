          ...
          <div className="mt-6 w-full max-w-[340px]">
            <div className="flex flex-col items-center gap-3">
              <div className="relative">
                <img src={resolvedTheme === 'dark' ? '/statics/favicon/logo.png' : '/statics/favicon/logo-dark.png'} alt="MaskPanel Logo" className="h-20 w-20 object-contain mask-glow rounded-2xl" />
                <div className="absolute -bottom-1 -right-1 h-6 w-6 rounded-full bg-gradient-to-br from-violet-600 to-cyan-600 flex items-center justify-center text-[10px] font-mono text-white">ᛗ</div>
              </div>
              <div className="flex flex-col items-center">
                <span className="text-2xl font-bold tracking-tight">
                  <span className="flux-text">Mask</span>Panel <span className="text-sm font-normal text-muted-foreground">by RunoFlux</span>
                </span>
                <span className="text-[11px] tracking-[0.2em] text-muted-foreground uppercase">Veiled Flux • ᚱᚢᚾᛟ ᚠᛚᚢᛉ</span>
              </div>
              <span className="text-xl font-semibold mt-2">{view === 'login' ? t('login.loginYourAccount') : t('setup.ownerAccess', { defaultValue: 'Owner access' })}</span>
              <span className="text-center text-muted-foreground text-sm">
                {view === 'login'
                  ? 'Veiled Connectivity, Flux Intelligence - Secure Access'
                  : t('setup.ownerAccessDescription', {
                      defaultValue: 'Use a temporary setup key to create, promote, reset, or remove the owner account.',
                    })}
              </span>
              {view === 'login' && (
                <div className="flex items-center gap-2 mt-2 px-3 py-1.5 rounded-full bg-gradient-to-r from-violet-500/10 to-cyan-500/10 border border-violet-500/20">
                  <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
                  <span className="text-[11px] font-mono">Ghost Mode: Ready • Flux: Optimal • 94% Obfuscated</span>
                </div>
              )}
            </div>
